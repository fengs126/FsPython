...
def main():
    ...
    window = tk.Tk()
    window.title(title)
    window.geometry('296x171')
    window.resizable(0, 0)

    tfile = tk.Text(window, width=34, height=2, state='disabled')
    tfile.place(x=0, y=0, anchor='nw')

    bclear = tk.Button(window, text='×', width=1, height=1, relief='groove', command=clearfile)
    bclear.place(x=237, y=0, anchor='nw')

    sfile = tk.StringVar()
    sfile.set('Open')
    bfile = tk.Button(window, textvariable=sfile, width=5, height=1, relief='groove', command=openfile)
    bfile.place(x=252, y=0, anchor='nw')

    ltext = tk.StringVar()
    lexamination = tk.Label(window, textvariable=ltext, bg='yellow', font=('Arial', 16), anchor='w', width=24, height=1)
    lexamination.place(x=1, y=31, anchor='nw')

    bentry = tk.Entry(window, width=42, state='disabled')
    bentry.place(x=0, y=62, anchor='nw')
    bentry.focus()
    bentry.bind_all('<Key-Return>', keyReturn)
    bentry.bind_all('<Key-Tab>', keyTab)

    tinfo = tk.Text(window, width=42, height=4, state='disabled')
    ...
    itop = tk.IntVar()
    itop.set(0)
    ctop = tk.Checkbutton(window, variable=itop, onvalue=1, offvalue=0, command=topmost)
    ctop.place(x=267, y=141, anchor='nw')
